...
export const EbookShelf: React.FC<EbookShelfProps> = ({ category, ebooks, onRead }) => {
  ...
  return (
    <section className="py-8 md:py-12 px-4 sm:px-6 lg:px-10 max-w-7xl mx-auto space-y-6 md:space-y-9">
      <div className="premium-shell px-4 md:px-6 py-5 md:py-6 space-y-4 md:space-y-5">
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
          ...
          <p className="text-[9px] uppercase tracking-[0.16em] md:tracking-[0.2em] text-black/45 font-bold md:mb-1">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-[1fr_auto_auto] gap-2 md:gap-3">
          ...
          <label className="relative">
            ...
            <select
              ...
              className="w-full h-10 md:h-11 pl-10 pr-7 bg-white/80 border border-black/10 text-[10px] md:text-xs uppercase tracking-[0.08em] md:tracking-[0.12em] outline-none transition-colors focus:border-[#C5A059]"
            >
          ...
          <select
            ...
            className="col-span-2 lg:col-span-1 h-10 md:h-11 px-3 bg-white/80 border border-black/10 text-[10px] md:text-xs uppercase tracking-[0.08em] md:tracking-[0.12em] outline-none transition-colors focus:border-[#C5A059]"
          >
      ...
      <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 lg:grid-cols-6 xl:grid-cols-8 gap-x-3 gap-y-6 md:gap-x-5 md:gap-y-8">
        {visible.map((ebook) => (
          <div
            key={ebook.id}
            className="group relative rounded-sm p-1.5 transition-colors hover:bg-white/55"
          ...
          >
            ...
            <div className="mt-2.5 md:mt-3 space-y-0.5 md:space-y-1">
              <p className={`text-[6px] md:text-[7px] uppercase font-black tracking-widest ${ebook.isSpecial ? 'accent-gold' : 'text-black/35'}`}>
                {ebook.category}
              </p>
              <p className="text-[9px] md:text-[10px] font-serif text-black/68 truncate">
                {ebook.author}
              </p>
      ...
      {filtered.length > visible.length && (
        <div className="flex justify-center pt-1 md:pt-2">
          <button
            ...
            className="h-10 md:h-11 px-5 md:px-6 premium-button-light text-[10px] uppercase tracking-[0.18em] md:tracking-[0.22em] font-bold hover:border-[#C5A059] hover:text-[#8A682B] transition-colors"
          >
            Carregar mais obras
          </button>
        </div>
